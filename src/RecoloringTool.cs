        ...
        : Tool
    {
        ...
        private void AdjustDrawingColor(MouseEventArgs e)
        {
            ...
            if (BtnDownMouseLeft(e))
            {
                ...
                this.Workspace.Environment.BackColor = AdjustColorDifference(oldColor, 
                    this.Workspace.Environment.ForeColor, this.Workspace.Environment.BackColor);
            }
            ...
            if (BtnDownMouseRight(e))
            {
                ...
                this.Workspace.Environment.ForeColor = AdjustColorDifference(oldColor, 
                    this.Workspace.Environment.BackColor, this.Workspace.Environment.ForeColor);
            }
        }
                                ...
                                if (srcBgra->A != 0)
                                {
                                    ...
                                    if (boolCIT || boolPAAA)
                                    {
                                        if (boolPAAA)
                                        {
                                            ...
                                            if (penWidth < 2.0f)
                                            {
                                                colorAdjusted.B = Utility.ClampToByte(colorReplacing.B + (colorAdjusted.B - colorToReplace.B));
                                                colorAdjusted.G = Utility.ClampToByte(colorReplacing.G + (colorAdjusted.G - colorToReplace.G));
                                                colorAdjusted.R = Utility.ClampToByte(colorReplacing.R + (colorAdjusted.R - colorToReplace.R));
                                                colorAdjusted.A = Utility.ClampToByte(colorReplacing.A + (colorAdjusted.A - colorToReplace.A));
                                            }
                                        }
                                        else
                                        {
                                            colorAdjusted.B = Utility.ClampToByte(colorLifted.B + (colorReplacing.B - colorToReplace.B));
                                            colorAdjusted.G = Utility.ClampToByte(colorLifted.G + (colorReplacing.G - colorToReplace.G));
                                            colorAdjusted.R = Utility.ClampToByte(colorLifted.R + (colorReplacing.R - colorToReplace.R));
                                            colorAdjusted.A = Utility.ClampToByte(colorLifted.A + (colorReplacing.A - colorToReplace.A));
                                        }
        ...
        protected override void OnMouseDown(MouseEventArgs e)
        {
            ...
            if (BtnDownMouseLeft(e) || BtnDownMouseRight(e))
            {
                ...
                if ((!KeyDownControlOnly()) && (!KeyDownShiftOnly()))
                {
                    ...
                    // establish tolerance
                    myTolerance = (int)(Workspace.Environment.Tolerance * 256);
