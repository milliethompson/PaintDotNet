...
namespace PaintDotNet
{
        ...
        : Tool
    {
        ...
        protected class CompoundToolHistoryAction
            : ToolHistoryAction
        {
            private CompoundHistoryAction cha;

            public CompoundHistoryAction CompoundHistoryAction
            {
                get
                {
                    return this.cha;
            ...
            {
                CompoundHistoryAction cha = (CompoundHistoryAction)this.cha.PerformUndo();
                CompoundToolHistoryAction ctha = new CompoundToolHistoryAction(cha, this.Workspace, this.Name, this.Image);
                return ctha;
            }

            public CompoundToolHistoryAction(CompoundHistoryAction cha, DocumentWorkspace workspace, string name, Image image)
        ...
        protected void DestroyNubs()
        {
            ...
            {
                this.Renderers.Remove(this.rotateNub);
        ...
        protected void PositionNubs(Mode currentMode)
        {
            ...
            if (this.rotateNub == null)
            {
                this.rotateNub = new RotateNubRenderer(this.Renderers);
                rotateNub.Visible = false;
                this.Renderers.Add(this.rotateNub, false);
            }
            ...
            else
            {
                foreach (MoveNubRenderer nub in this.moveNubs)
                {
                    ...
                    nub.Transform = Workspace.Environment.Selection.GetInterimTransformReadOnly();
                }
            }
        }
        ...
        protected void ConstrainScaling(RectangleF liftedBounds, float startWidth, float startHeight, float newWidth, float newHeight, 
            out float newXScale, out float newYScale)
        {
        ...
        protected override void OnKeyPress(Keys key)
        {
            if (!tracking)
            {
                ...
                if (dx != 0 || dy != 0)
                {
                    ...
                    Point docPos = new Point(-70000, -70000);//Workspace.DocumentView.ScreenToDocument(pos);
        ...
        protected abstract void PushContextHistoryAction();

        protected void Lift(MouseEventArgs e)
        {
            this.PushContextHistoryAction();
            ...
            this.context.liftTransform = Workspace.Environment.Selection.GetCumulativeTransformCopy();
        ...
        protected override void OnMouseDown(MouseEventArgs e)
        {
            ...
            if (newMode == Mode.Translate ||
                ...
                newMode == Mode.Rotate)
            {
                this.context.startBounds = Workspace.Environment.Selection.GetBoundsF();
                this.context.startMouseXY = new Point(e.X, e.Y);
                this.context.offset = new Point(0, 0);
                ...
                this.context.baseTransform = Workspace.Environment.Selection.GetInterimTransformCopy();
            }
            ...
            this.context.startPath = Workspace.Environment.Selection.CreatePath();
            this.context.startAngle = Utility.GetAngleOfTransform(Workspace.Environment.Selection.GetInterimTransformReadOnly());

            SelectionHistoryAction sha1 = new SelectionHistoryAction(this.Name, this.Image, this.Workspace);
            this.currentHistoryActions.Add(sha1);
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            else
            {
                ...
                using (Matrix translateMatrix = new Matrix())
                {
                    ...
                    if (this.context.baseTransform != null)
                    {
                        Workspace.Environment.Selection.SetInterimTransform(this.context.baseTransform);
                    }

                    Matrix interim = Workspace.Environment.Selection.GetInterimTransformCopy();
        ...
        {
            Workspace.EnableSelectionOutline = true;
            base.OnMouseUp (e);
        }

        public MoveToolBase(DocumentWorkspace workspace, Image toolBarImage, string name, string helpText, char hotKey)
